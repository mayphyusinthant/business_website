<script setup>
    ...
    const { items, categories,  filteredCategory } = defineProps(['items', 'categories',  'filteredCategory']);
    console.log('Items Data' , items.data);
    console.log('Category' , categories);
    console.log('First Page URL' , items.first_page_url);
    console.log('Last Page URL' , items.last_page_url);
    console.log('Links' , items.links);
    console.log('Next Page URL' , items.next_page_url);
  ...
  if (url) {
    ...
    const deleteItem = (item) => {
      ...
      form.delete(route('admin.delete',  { id: item.id }), {
          preserveScroll: true,
           ...
           <div class="grid grid-cols-5 items-center">
            ...
            <form  class="col-span-1 flex items-center space-x-2">
                ...
                >
                    ...
                    <option value="filteredCategory"  selected>{{ filteredCategory ? filteredCategory : "Filter By Trophy Type" }}</option>
            ...
            <div class="col-span-1 w-full flex justify-end">
                <button class="bg-pink-900 text-white py-2 px-4 rounded hover:bg-pink-950 focus:shadow-outline focus:outline-none"
                    ...
                    <Link :href="route('admin.create')">Create New Item</Link>
                </button>
            </div>
        ...
        <div class="py-12" >
          <div class="flex justify-center items-center mt-6">
              ...
              <ul class="pagination flex">
                  ...
                  <li v-for="link in items.links" :key="link.label">
                    <template v-if="link.url">
                        ...
                        <a v-if="link.label === items.current_page || Math.abs(link.label - items.current_page) <= 1"
                            :href="link.url"
                        ...
                        <a :href="link.url" v-else-if="Math.abs(link.label - items.current_page) <= 2"
                        ...
                        <a :href="link.url" v-else-if="Math.abs(link.label - items.current_page) === 3"
                            class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700">...</a>
                        <a :href="items.prev_page_url" v-else-if="link.label.includes('Previous')"
                        ...
                        <a :href="items.next_page_url" v-else-if="link.label.includes('Next')"
                        v-html="link.label" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700">  </a>
           ...
           <!-- <nav aria-label="Page navigation"  class = "flex items-center justify-center "  >
            ...
            <div class="max-w-8xl mx-auto  p-5 sm:px-6 lg:px-8  overflow-hidden shadow-xl sm:rounded-lg" :class="{ 'dark:bg-gray-800': darkMode }">
              
              <Banner :v-if="show && message"/>  

              <!-- Table -->
              Page {{ items.current_page }} of Total {{ items.last_page }} Pages
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 ">
                    <thead class="text-xs text-white uppercase bg-pink-900" :class="{ 'dark:bg-gray-800': darkMode }">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3">
                                <div class="flex items-center">Category
                                    <a href="#"><svg class="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z"/>
                                      </svg>
                                    </a>
                            ...
                            <th scope="col" class="px-6 py-3">
                              <div class="flex items-center">Medium
                                  <a href="#"><svg class="w-3 h-3 ms-1.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                      <path d="M8.574 11.024h6.852a2.075 2.075 0 0 0 1.847-1.086 1.9 1.9 0 0 0-.11-1.986L13.736 2.9a2.122 2.122 0 0 0-3.472 0L6.837 7.952a1.9 1.9 0 0 0-.11 1.986 2.074 2.074 0 0 0 1.847 1.086Zm6.852 1.952H8.574a2.072 2.072 0 0 0-1.847 1.087 1.9 1.9 0 0 0 .11 1.985l3.426 5.05a2.123 2.123 0 0 0 3.472 0l3.427-5.05a1.9 1.9 0 0 0 .11-1.985 2.074 2.074 0 0 0-1.846-1.087Z"/>
                                    </svg>
                                  </a>
                  ...
                  <tbody>
                    ...
                    <template v-for="item in items.data" :key="item.id">
                        ...
                        <tr  class="border-b text-pink-950"  :class="{ 'dark:bg-gray-450': darkMode  , 'dark:text-white': darkMode  }">
                          ...
                          <td class="px-6 py-4">
                            {{ dateFormattter(item.created_at) }}
                        </td>
                        <td class="px-6 py-4">
                          {{ dateFormattter(item.updated_at) }}
                      </td>
                            <td class="px-6 py-4 text-right">
                              ...
                              <div class="mt-0">
                                <button  @click="confirmItemDeletion"
                            ...
                            <DialogModal :show="confirmingItemDeletion" @close="closeModal">
                                <template #title>
                                    Delete Item
                                </template>
